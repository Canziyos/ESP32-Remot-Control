...
#include "ble_ota_stub.h"     // declares the hooks we override
...
void ble_ota_on_ctrl_write(const uint8_t *data, uint16_t len)
{
    ...
    if (strncmp(line, "BL_OTA START", 12) == 0) {
        ...
        // (Optional policy) Gate by mode:
        // if (syscoord_get_mode() != SC_MODE_RECOVERY) { ble_tx_send("ERR FORBIDDEN"); return; }
