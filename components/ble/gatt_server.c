...
static ble_cmd_t *s_cli = NULL;  // created once in init.
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
    ...
    case ESP_GATTS_START_EVT:
        ESP_LOGI(TAG, "Service started. Notifying syscoord.");
    ...
    case ESP_GATTS_DISCONNECT_EVT:
        ...
        if (s_cli) ble_cmd_on_disconnect(s_cli);
        g_conn_id = 0xFFFF; 
        break;
...
void gatt_server_init(void) {
    ...
    // Create BLE CLI once; reuse per connection.
    if (!s_cli) s_cli = ble_cmd_create();
